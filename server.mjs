...
function streamRun(req, res, { id, title, file = null, entry, args = [], task = null }) {
  ...
  // One writer at a time. In-process exclusion first (specific message), then the
  // ATOMIC cross-instance lock — acquired BEFORE spawning so two instances sharing
  // this checkout can't both win the race.
  ...
  writeRunLock(id, child.pid); // replace the placeholder pid with the real child pid (atomic)
  ...
  const mirror = (line) => { // additive bus tap — the per-run SSE stream is untouched
    ...
    // ---- loop-pipeline phases (additive; the classic runner never emits these) ----
      ...
      publish('run.done',
        ...
        { taskId: id, ok: !!o.ok, verified: !!o.verified, reason: o.reason, branch: o.branch, pr: o.pr, note: o.note, retries: o.retries, checkerNotes: o.notes });
      setLastRun({ taskId: id, title, ok: !!o.ok, verified: !!o.verified, reason: o.reason || null, branch: o.branch || null, pr: o.pr || null, note: o.note || null });
  ...
  req.on('close', () => {
    // SIGTERM lets the child run its cleanup (kills the agent tree, releases the
    // lock) and exit; escalate to SIGKILL if it's still alive after a grace.
